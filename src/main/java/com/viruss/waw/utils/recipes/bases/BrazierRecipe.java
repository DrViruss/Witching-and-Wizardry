...
public class BrazierRecipe implements Recipe<SimpleContainer> {
    ...
    public BrazierRecipe(ResourceLocation id, ItemStack result,AbstractRitualAction action,int time ,int cost ,Pair<String,Integer>[] circles,List<Pair<Integer,Integer>> candles, Ingredient... ingredients) {
        if(action == null) throw new InvalidValueException("Please add ACTION to brazier"+id.toString());
        this.id = id;
    ...
    public boolean matches(SimpleContainer container, Level level) {
        ...
        for (int j = 0; j < container.getContainerSize(); j++) {
            ...
            if (!item.isEmpty() && (!ingredients.test(item) || item.getItem() instanceof Chalk && !testChalk(ingredients.getItems(),item)))
                return false;
        }
        return true;
    }
...
//    public boolean testAdditional(Level level, Player player, BlockPos centerPos){
        ...
        // add: altar, familiar
//
//        return (circles == null || testArea(level,centerPos)) && (candles == null || testCandles(level, centerPos));
//    }
//
...
//    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return RecipeTypes.Brazier.SERIALIZER;
    }

    @Override
    public RecipeType<?> getType() {
        return RecipeTypes.Brazier.TYPE;
    }
    ...
    public static class FinishedRecipe implements net.minecraft.data.recipes.FinishedRecipe {
        ...
        public FinishedRecipe(Item result, AbstractRitualAction action , int time, int cost, Pair<String,Integer>[] circles, List<Pair<Integer,Integer>> candles, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+ Objects.requireNonNull(result.getRegistryName()).getPath());
        ...
        public FinishedRecipe(ItemStack result,AbstractRitualAction action ,int time,int cost,Pair<String,Integer>[] circles,List<Pair<Integer,Integer>> candles, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(result.getItem().getRegistryName()).getPath());
            this.input = ingredients;
            this.output = result;
            this.circles=circles;
            this.candles=candles;
        ...
        public FinishedRecipe(ItemStack result,AbstractRitualAction action ,int time,int cost,Pair<String,Integer>[] circles, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(result.getItem().getRegistryName()).getPath());
            this.input = ingredients;
            this.output = result;
            this.circles=circles;
        ...
        public FinishedRecipe(ItemStack result,AbstractRitualAction action ,int time, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(result.getItem().getRegistryName()).getPath());
            this.input = ingredients;
            this.output = result;
            this.circles=null;
        ...
        public FinishedRecipe(AbstractRitualAction action, int time,int cost,Pair<String,Integer>[] circles,List<Pair<Integer,Integer>> candles, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(action.getRegistryName()).getPath().replaceFirst("_action",""));
            this.input = ingredients;
            this.circles=circles;
            this.candles=candles;
            this.time=time;
        ...
        public FinishedRecipe(AbstractRitualAction action, int time,int cost,Pair<String,Integer>[] circles, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(action.getRegistryName()).getPath().replaceFirst("_action",""));
            this.input = ingredients;
            this.circles=circles;
            this.candles=null;
        ...
        public FinishedRecipe(AbstractRitualAction action, int time, Ingredient... ingredients) {
            this.id = new ResourceLocation(Main.MOD_ID,"brazier/"+Objects.requireNonNull(action.getRegistryName()).getPath().replaceFirst("_action",""));
            this.input = ingredients;
            this.circles=null;
            this.candles=null;
    ...
    public static class Serializer extends ForgeRegistryEntry<RecipeSerializer<?>> implements RecipeSerializer<BrazierRecipe> {

        public Serializer() {
            setRegistryName(Main.MOD_ID, "brazier");
        }
...
//                candles = new Pair<>(tmp.get("color").getAsInt(),tmp.get("amount").getAsInt());
            }
//ShapelessRecipe
            ...
            return new BrazierRecipe(
                    id,
                    json.has("result") ? ShapedRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(json, "result")) : ItemStack.EMPTY,
                    action,
                    time,
                    cost,
                    tmp != null ? new Pair[]{
        ...
        @Nullable
        @Override
        public BrazierRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buffer) {
        ...
        public void toNetwork(FriendlyByteBuf buffer, BrazierRecipe recipe) {
            ...
            buffer.writeResourceLocation(recipe.action.getRegistryName());
            buffer.writeInt(recipe.time);
            buffer.writeByte(recipe.cost);

            CompoundTag tag = new CompoundTag();
            if(recipe.getCirclesCount() > 0) {
                tag.putBoolean("circles",true);
